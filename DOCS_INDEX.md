# Documentation Index

Organized documentation for ocNewsBot project.

## Main Documents

### Essential Reading
- **[README.md](README.md)** - Project overview and quick start
- **[DEV_GUIDE.md](DEV_GUIDE.md)** - Complete developer guide (setup, testing, troubleshooting)
- **[AI_CONTEXT.md](AI_CONTEXT.md)** - Token-efficient context for AI agents

### Deployment Guides
- **[DEPLOY_VPS.md](DEPLOY_VPS.md)** - 🚀 Step-by-step VPS deployment (Ubuntu/Debian)
- **[README.Docker.md](README.Docker.md)** - 🐳 Docker deployment reference
- **[ENV_REFERENCE.md](ENV_REFERENCE.md)** - Complete environment variables guide

### Reference Specs
- **[srs.md](srs.md)** - Software Requirements Specification
- **[ARCHITECHTURE.md](ARCHITECHTURE.md)** - System architecture details

### Feature-Specific Guides
- **[AI_SETUP.md](AI_SETUP.md)** - OpenAI configuration and testing
- **[DIGEST.md](DIGEST.md)** - Digest generation service
- **[TESTING.md](TESTING.md)** - Pipeline testing instructions

## Component Documentation

### Backend (server/)
- **[server/API.md](server/API.md)** - Complete API reference

### Frontend (web/)
- All frontend documentation consolidated in DEV_GUIDE.md

## Documentation Purpose Guide

**If you want to:**
- Get started quickly → **[README.md](README.md)** + **[DEV_GUIDE.md](DEV_GUIDE.md)**
- Deploy to VPS → **[DEPLOY_VPS.md](DEPLOY_VPS.md)**
- Deploy with Docker → **[README.Docker.md](README.Docker.md)**
- Configure environment → **[ENV_REFERENCE.md](ENV_REFERENCE.md)**
- Understand requirements → **[srs.md](srs.md)**
- Understand architecture → **[ARCHITECHTURE.md](ARCHITECHTURE.md)** or **[AI_CONTEXT.md](AI_CONTEXT.md)**
- Setup AI pipeline → **[AI_SETUP.md](AI_SETUP.md)**
- Test the system → **[TESTING.md](TESTING.md)** + **[DEV_GUIDE.md](DEV_GUIDE.md)**
- Understand digest service → **[DIGEST.md](DIGEST.md)**
- Use API endpoints → **[server/API.md](server/API.md)** or **[DEV_GUIDE.md](DEV_GUIDE.md)**
- Setup database/backend/frontend → **[DEV_GUIDE.md](DEV_GUIDE.md)**

## For AI Agents

When working with AI coding assistants, provide:
1. **[AI_CONTEXT.md](AI_CONTEXT.md)** - Essential project context (token-efficient)
2. **[DEV_GUIDE.md](DEV_GUIDE.md)** - If agent needs setup/testing info

These two files contain all critical information in optimized format.
