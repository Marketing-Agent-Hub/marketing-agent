...
**Query Parameters:**
- `status` (optional): `NEW`, `EXTRACTED`, `FILTERED_OUT`, `READY_FOR_AI`, `AI_STAGE_A_DONE`, `AI_STAGE_B_DONE`, `USED_IN_POST`, `REJECTED`
...
Lấy thống kê về pipeline processing.
...
```json
{
  ...
  "processingStats": {
    ...
    "avgFilteringTime": 2.1,
    "avgAiStageATime": 8.5,
    "avgAiStageBTime": 15.3
  },
  "successRates": {
    "extraction": 95.5,
    "aiStageA": 78.2,
  ...
  "activities": [
    {
      ...
      "sourceId": 1,
      "status": "SUCCESS",
      "details": "Successfully ingested new item from VnExpress"
    },
    {
      "timestamp": "2026-03-04T10:29:00.000Z",
      "type": "AI_STAGE_B",
      "itemId": 99,
      "status": "SUCCESS",
      "details": "Generated Facebook post"
    }
  ]
}
```
...
Phát hiện bottlenecks trong pipeline.
  ...
  "bottlenecks": [
    {
      ...
      "estimatedTimeToComplete": "6.2 hours",
      "severity": "HIGH"
    },
    {
      "stage": "EXTRACTION",
      ...
      "estimatedTimeToComplete": "2.9 hours",
      "severity": "MEDIUM"
    }
  ],
  "recommendations": [
    "Consider increasing AI processing frequency",
...
async function checkPipelineStatus() {
  // Get pipeline stats
  const statsResponse = await fetch('http://localhost:3000/api/stats/pipeline', {
    headers: {
      'Authorization': `Bearer ${token}`,
    },
  });
  ...
  console.log(`- NEW items: ${stats.itemsByStatus.NEW}`);
  console.log(`- EXTRACTED: ${stats.itemsByStatus.EXTRACTED}`);
  console.log(`- READY_FOR_AI: ${stats.itemsByStatus.READY_FOR_AI}`);
  console.log(`- AI_STAGE_B_DONE: ${stats.itemsByStatus.AI_STAGE_B_DONE}`);
  
  // Check for bottlenecks
  const bottlenecksResponse = await fetch('http://localhost:3000/api/stats/bottlenecks', {
    headers: {
      'Authorization': `Bearer ${token}`,
    },
  });
  
  const bottlenecks = await bottlenecksResponse.json();
  
  if (bottlenecks.bottlenecks.length > 0) {
    console.log('\nBottlenecks detected:');
    bottlenecks.bottlenecks.forEach(b => {
      console.log(`- ${b.stage}: ${b.queueSize} items (${b.estimatedTimeToComplete})`);
    });
  }
}
...
### Workflow 5: Search và Filter Items
