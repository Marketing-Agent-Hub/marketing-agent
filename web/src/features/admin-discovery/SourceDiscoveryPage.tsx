...
export default function SourceDiscoveryPage() {
    ...
    return (
        <div className="mx-auto max-w-xl font-mono">
            ...
            {current && (
                <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-6 space-y-4 transition-colors">
                    <div>
                        <h2 className="text-base font-bold text-[var(--color-text)]">{current.name}</h2>
                        <a href={current.url} target="_blank" rel="noopener noreferrer"
                            className="text-xs text-[#4FACFE] hover:underline truncate block">{current.url}</a>
                    </div>

                    <div>
                        <p className="mb-2 text-[10px] uppercase tracking-wider text-[var(--color-text-muted)] opacity-50">Sample Headlines</p>
                        <ul className="space-y-1">
                            {current.sampleHeadlines.map((h, i) => (
                                <li key={i} className="text-xs text-[var(--color-text)] opacity-80">• {h}</li>
                            ))}
                        </ul>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-[10px] text-[var(--color-text-muted)] opacity-50">Relevance Score</p>
            ...
            <p className="mt-4 text-center text-[10px] text-[var(--color-text-muted)] opacity-40">
                <kbd className="rounded border border-[var(--color-border)] px-1 bg-[var(--color-bg-card)]">Y</kbd> Approve ·{' '}
                <kbd className="rounded border border-[var(--color-border)] px-1 bg-[var(--color-bg-card)]">N</kbd> Reject
            </p>
        </div>
    );
}
